...
| Amp | `local_import_when_supported` | Official explicit JSON emitted by `amp threads export <threadIDOrURL>` and imported with `ctx import --provider amp --path <file>`. | Native explicit-export support; static redacted export fixture smoke; no default discovery is registered, and `$XDG_CACHE_HOME/amp/logs/cli.log` is not crawled. |
...
Amp uses `import_support: "explicit"` for the official `amp threads export`
JSON contract. The support matrix gives Devin ATIF the same hard
explicit-export status semantically: both require a user-supplied `--path`, have
no default source discovery, and are not native-auto.
