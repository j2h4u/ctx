...
Updated: 2026-06-22T22:59:00-05:00
...
| macOS shared Buildkite checkout directories can contain stale files that the agent user cannot remove. | macOS lanes can fail before running repo commands. | macOS smoke/release lanes now set `BUILDKITE_BUILD_CHECKOUT_PATH` to isolated `/tmp` per-build paths and use Buildkite `custom-checkout#v1.8.0` with `skip_checkout: true` and `delete_checkout: true`, so the external pre-command hook sees the checked-out repo without touching stale shared paths. |
| PowerShell argument binding differs from Bash array passing. | Windows CI wrappers can accidentally invoke bare tools even when local shell scripts pass. | `scripts/ci-windows.ps1` avoids the automatic `$Args` name by using `CargoArgs`/`CtxArgs`, and the Buildkite pipeline contract asserts the named `Run-Cargo -CargoArgs` call shape. |
